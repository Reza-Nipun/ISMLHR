...
//	LV Employee List by- BuN=ISML (DayWisePayHour Copy from MSSQL to MySQL)	///
$SQL="SELECT ph.EmployeeCode, ph.WorkDate, ph.ARADayStatus, einfo.Unit, einfo.[Staff Category] as sc from DayWisePayHour ph INNER JOIN EmployeePIMSinfo einfo ON einfo.EmployeeCode=ph.EmployeeCode WHERE einfo.BuN='ISML' AND einfo.EmployeeStatus='Active' AND ph.WorkDate='$today' AND ph.ARADayStatus='LV'";
		$result = mssql_query($SQL);
		while($row = mssql_fetch_array($result))
		{ 
			$EmployeeCode=$row['EmployeeCode'];
...
/////		KORMEE to MySQL	End	///////////
//	Active	List	Unit-1	///
$sql="SELECT COUNT(DISTINCT EmployeeCode) AS sum, StaffCategory as sc FROM tb_isml_employeepimsinfo WHERE Unit='UNIT-09' GROUP BY StaffCategory";

		$result = mysql_query($sql);
		
		while($row = mysql_fetch_array($result))
                            { 
							 if(($row["sc"])=='FW')$FW1=$row['sum'];
							 if(($row["sc"])=='NMS')$NMS1=$row['sum'];
							 if(($row["sc"])=='MS')$MS1=$row['sum'];
							}

$strength_unit1=($FW1+$NMS1+$MS1);
//	Active	List	Unit-2	///
$sql="SELECT COUNT(DISTINCT EmployeeCode) AS sum, StaffCategory as sc FROM tb_isml_employeepimsinfo WHERE Unit='UNIT-09-U2' GROUP BY StaffCategory";

		$result = mysql_query($sql);
		
		while($row = mysql_fetch_array($result))
                            { 
							 if(($row["sc"])=='FW')$FW2=$row['sum'];
							 if(($row["sc"])=='NMS')$NMS2=$row['sum'];
							 if(($row["sc"])=='MS')$MS2=$row['sum'];
							}
$strength_unit2=($FW2+$NMS2+$MS2);
$strength_total=($strength_unit1+$strength_unit2);
//	Present	List	Unit-1		///
$sql="SELECT COUNT(DISTINCT att.EmployeeCode) AS sum, einfo.StaffCategory as sc from tb_isml_attendance att INNER JOIN tb_isml_employeepimsinfo einfo ON einfo.EmployeeCode=att.EmployeeCode WHERE einfo.Unit='UNIT-09' AND att.WorkDate='$today' GROUP BY einfo.StaffCategory";
//die($sql);
		$result = mysql_query($sql);
		
		while($row = mysql_fetch_array($result))
                            { 
							 if(($row["sc"])=='FW')$present_FW1=$row['sum'];
							 if(($row["sc"])=='NMS')$present_NMS1=$row['sum'];
							 if(($row["sc"])=='MS')$present_MS1=$row['sum'];
							}
$present_unit1=($present_FW1+$present_NMS1+$present_MS1);
//	Present	List	Unit-2		///
$sql="SELECT COUNT(DISTINCT att.EmployeeCode) AS sum, einfo.StaffCategory as sc from tb_isml_attendance att INNER JOIN tb_isml_employeepimsinfo einfo ON einfo.EmployeeCode=att.EmployeeCode WHERE einfo.Unit='UNIT-09-U2' AND att.WorkDate='$today' GROUP BY einfo.StaffCategory";
		//die($SQL);
		$result = mysql_query($sql);
		
		while($row = mysql_fetch_array($result))
                            { 
							 if(($row["sc"])=='FW')$present_FW2=$row['sum'];
							 if(($row["sc"])=='NMS')$present_NMS2=$row['sum'];
							 if(($row["sc"])=='MS')$present_MS2=$row['sum'];
							}
$present_unit2=($present_FW2+$present_NMS2+$present_MS2);
$present_total=($present_unit1+$present_unit2);
//	Leave	List	Unit-1		///
$sql="SELECT COUNT(DISTINCT EmployeeCode) AS sum, StaffCategory as sc FROM tb_isml_lv WHERE Unit='UNIT-09' GROUP BY StaffCategory";
		//die($SQL);
		$result = mysql_query($sql);
		
		while($row = mysql_fetch_array($result))
                            { 
							 if(($row["sc"])=='FW')$LV_FW1=$row['sum'];
							 if(($row["sc"])=='NMS')$LV_NMS1=$row['sum'];
							 if(($row["sc"])=='MS')$LV_MS1=$row['sum'];
							}
$LV_unit1=($LV_FW1+$LV_NMS1+$LV_MS1);
//	Leave	List	Unit-2		///
$sql="SELECT COUNT(DISTINCT EmployeeCode) AS sum, StaffCategory as sc FROM tb_isml_lv WHERE Unit='UNIT-09-U2' GROUP BY StaffCategory";
		//die($SQL);
		$result = mysql_query($sql);
		
		while($row = mysql_fetch_array($result))
                            { 
							 if(($row["sc"])=='FW')$LV_FW2=$row['sum'];
							 if(($row["sc"])=='NMS')$LV_NMS2=$row['sum'];
							 if(($row["sc"])=='MS')$LV_MS2=$row['sum'];
							}
$LV_unit2=($LV_FW2+$LV_NMS2+$LV_MS2);
$LV_total=($LV_unit1+$LV_unit2);

$absent_FW1=$FW1-($present_FW1+$LV_FW1);
$absent_NMS1=$NMS1-($present_NMS1+$LV_NMS1);
$absent_MS1=$MS1-($present_MS1+$LV_MS1);
$absent_unit1=($absent_FW1+$absent_NMS1+$absent_MS1);

$absent_FW2=$FW2-($present_FW2+$LV_FW2);
$absent_NMS2=$NMS2-($present_NMS2+$LV_NMS2);
$absent_MS2=$MS2-($present_MS2+$LV_MS2);
$absent_unit2=($absent_FW2+$absent_NMS2+$absent_MS2);

$absent_total=($absent_unit1+$absent_unit2);


if($FW1!=0)$attnd_unit1_FW=round(($present_FW1/$FW1)*100,2)." %";
if($FW1!=0)$attnd_unit1_NMS=round(($present_NMS1/$NMS1)*100,2)." %";
if($FW1!=0)$attnd_unit1_MS=round(($present_MS1/$MS1)*100,2)." %";

if($strength_unit1!=0)$attnd_unit1_percentage=round(($present_unit1/$strength_unit1)*100,2)." %";

if($FW2!=0)$attnd_unit2_FW=round(($present_FW2/$FW2)*100,2)." %";
if($FW2!=0)$attnd_unit2_NMS=round(($present_NMS2/$NMS2)*100,2)." %";
if($FW2!=0)$attnd_unit2_MS=round(($present_MS2/$MS2)*100,2)." %";

if($strength_unit2!=0)$attnd_unit2_percentage=round(($present_unit2/$strength_unit2)*100,2)." %";

if($strength_total!=0)$total_present_percentage=round(($present_total/$strength_total)*100,2)." %";
if($strength_total!=0)$total_absent_percentage=round(($absent_total/$strength_total)*100,2)." %";
if($strength_total!=0)$total_leave_percentage=round(($LV_total/$strength_total)*100,2)." %";
...
?>
      ...
      <tr align="center">
            ...
            <table width="65%" cellpadding="0" cellspacing="0">
               <tr>
                  <td style="background-color:#025C64; color:#FFF; padding: 14px 12px 14px 12px">
                     <span style="font-size: 38px;">ISML Daily Attendance Summary</span>&nbsp;&nbsp;(Test Run)
                  </td>
                    ...
                    <table align="center" border="1" width="90%" cellspacing="0" cellpadding="0" class="bottomBorder" style="line-height: 20px">
                      ...
                      <tr align="center" style="font-size:14px">
                        ...
                        <tr style="font-size:14px">
                            ...
                            <th scope="row"><?php echo $LV_NMS1; ?></th>
                        </tr>
                        <tr style="font-size:14px">
                            <th scope="row">MS</th>
                            ...
                            <th scope="row"><?php echo $LV_MS1; ?></th>
                        </tr>
                        <tr style="font-size:14px">
...
<!--	Unit-2 Report	-->
                        ...
                        <tr style="font-size:14px">
                            ...
                            <th scope="row"><?php echo $LV_NMS2; ?></th>
                        </tr>
                        <tr style="font-size:14px">
                            <th scope="row">MS</th>
                            ...
                            <th scope="row"><?php echo $LV_MS2; ?></th>
                        </tr>
                        <tr style="font-size:14px">
                            <th scope="row">Total</th>
         ...
         <!--	Summary Report		-->
                    ...
                    <table align="center" border="1" width="40%" cellspacing="0" cellpadding="0" class="bottomBorder" style="line-height: 20px">
                        <tr style="font-size:14px" bgcolor="#0099FF">
                            <th width="18%" scope="row">Grant Total</th>
