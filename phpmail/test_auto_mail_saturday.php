...
/////	MANUAL att. ISML-01		\\\\\\\\\\\\\
        $SQL_PRESENT_MANUAL1 = "SELECT COUNT(DISTINCT dp.EmployeeCode) AS sum, ep.[Staff Category] as sc from DaywisePayHour dp , EmployeePIMSinfo ep WHERE ep.EmployeeCode=dp.EmployeeCode AND ep.Unit='UNIT-09' AND ep.EmployeeStatus='Active' AND dp.WorkDate='2014-09-02' GROUP BY ep.[Staff Category]";

        $result_str_manual1 = mssql_query($SQL_PRESENT_MANUAL1);

        while ($row = mssql_fetch_array($result_str_manual1)) {
            if (($row["sc"]) == 'FW'){
            $present_FW_manual1 = $row['sum'];}
            if (($row["sc"]) == 'NMS'){
            $present_NMS_manual1 = $row['sum'];}
            if (($row["sc"]) == 'MS'){
            $present_MS_manual1 = $row['sum'];}
        }
echo "ISML-01:<br>";
echo "FW=".$present_FW_manual1."<br>";
echo "NMS=".$present_NMS_manual1."<br>";
echo "MS=".$present_MS_manual1."<br>";
//die("OK");
////
        $present_unit1 = ($present_FW1 + $present_NMS1 + $present_MS1);

$PRESENT_ROW_MANUAL_Unit1_FW = $present_FW1+$present_FW_manual1;
$PRESENT_ROW_MANUAL_Unit1_NMS = $present_NMS1+$present_NMS_manual1;
$PRESENT_ROW_MANUAL_Unit1_MS = $present_MS1+$present_MS_manual1;

$PRESENT_ROW_MANUAL_Unit1_All = ($PRESENT_ROW_MANUAL_Unit1_FW + $PRESENT_ROW_MANUAL_Unit1_NMS + $PRESENT_ROW_MANUAL_Unit1_MS);
...
/////	MANUAL att. ISML-02		\\\\\\\\\\\\\
        $SQL_PRESENT_MANUAL2 = "SELECT COUNT(DISTINCT dp.EmployeeCode) AS sum, ep.[Staff Category] as sc from DaywisePayHour dp , EmployeePIMSinfo ep WHERE ep.EmployeeCode=dp.EmployeeCode AND ep.Unit='UNIT-09-U2' AND ep.EmployeeStatus='Active' AND dp.WorkDate='2014-09-02' GROUP BY ep.[Staff Category]";

        $result_str_manual2 = mssql_query($SQL_PRESENT_MANUAL2);

        while ($row = mssql_fetch_array($result_str_manual2)) {
            if (($row["sc"]) == 'FW'){
            $present_FW_manual2 = $row['sum'];}
            if (($row["sc"]) == 'NMS'){
            $present_NMS_manual2 = $row['sum'];}
            if (($row["sc"]) == 'MS'){
            $present_MS_manual2 = $row['sum'];}
        }
echo "ISML-02:<br>";
echo "FW=".$present_FW_manual2."<br>";
echo "NMS=".$present_NMS_manual2."<br>";
echo "MS=".$present_MS_manual2."<br>";
die("OK");
////
        $present_unit2 = ($present_FW2 + $present_NMS2 + $present_MS2);

$PRESENT_ROW_MANUAL_Unit2_FW = $present_FW2+$present_FW_manual2;
$PRESENT_ROW_MANUAL_Unit2_NMS = $present_NMS2+$present_NMS_manual2;
$PRESENT_ROW_MANUAL_Unit2_MS = $present_MS2+$present_MS_manual2;

$PRESENT_ROW_MANUAL_Unit2_All = ($PRESENT_ROW_MANUAL_Unit2_FW + $PRESENT_ROW_MANUAL_Unit2_NMS + $PRESENT_ROW_MANUAL_Unit2_MS);

		$present_total = ($PRESENT_ROW_MANUAL_Unit1_All + $PRESENT_ROW_MANUAL_Unit2_All);
...
if ($present_total == 0)
	{ 
		...
		$bcc_mail_list = array('shohal.bhuiyan@interfabshirt.com');
	}
...
//$toemail= array('masud.uddin@interfabshirt.com', 'abdullah.almamun@interfabshirt.com', 'nazrul.chowdhury@interfabshirt.com');
//$cc_mail_list = array('arif.abdulla@interfabshirt.com', 'aleya.aktar@interfabshirt.com', 'khandakar.rafiq@viyellatexgroup.com', 'masum.ikbal@viyellatexgroup.com');
...
if ($present_total == 0)
	...
	{ 
		$toemail= array('ahasan-interfab@viyellatexgroup.com', 'fateh@interfabshirt.com');
		$cc_mail_list = array('rana.shohel@viyellatexgroup.com', 'khandakar.rafiq@viyellatexgroup.com', 'arif.abdulla@interfabshirt.com', 'aleya.aktar@interfabshirt.com', 'shakil.mahmud@interfabshirt.com', 'masud.uddin@interfabshirt.com', 'nazrul.chowdhury@interfabshirt.com');
		
		$bcc_mail_list = array('shohal.bhuiyan@interfabshirt.com', 'mahbub.rahman@viyellatexgroup.com');
