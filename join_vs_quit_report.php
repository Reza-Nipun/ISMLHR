...
$where = '';
if($type == 'JOIN'){
    $where .= " AND MONTH([DOJ])= '$mon' AND YEAR([DOJ])= YEAR(getdate())";
}
if($type == 'QUIT'){
    $where .= " AND MONTH([DOS])= '$mon' AND YEAR([DOS])= YEAR(getdate())";
}
...
<div class="container">
    <div class="row">
        ...
        <div class="text-center col-md-3">
            <label class="form-control"><?php echo $month;?></label>
        </div>
    </div>
...
<br />
    ...
    <div class="row">
        ...
        <div class="col-md-1">
            <button class="btn btn-success" name="search_btn" id="search_btn" onclick="getFilterJoinQuitReport('<?php echo $emp_type;?>', '<?php echo $unit;?>', '<?php echo $date;?>', '<?php echo $type;?>', '<?php echo $mon;?>');">Search</button>
        </div>
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            <?php
                            if($type == 'JOIN'){
                                echo date('Y-m-d', strtotime($row['DOJ']));
                            }
                            if($type == 'QUIT'){
...
<script type="text/javascript">
    ...
    function getFilterJoinQuitReport(emp_type, unit, date, type, mon){
        ...
        if(type == 'JOIN'){
            $.ajax({
                url: "join_filter_report.php",
                type: "POST",
                data: {emp_type: emp_type, unit: unit, date: date, floor: floor, department: department, designation: designation, line_info: line_info, mon: mon},
                dataType: "html",
